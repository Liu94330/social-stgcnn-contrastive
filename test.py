...
from utils import * 
from metrics import * 
from model import social_stgcnn_with_contrastive
import copy
...
def test(KSTEPS=20):
    ...
    for batch in loader_test: 
        ...
        V_pred, _, total_contrastive_loss = model(V_obs_tmp, A_obs.squeeze())  # 获取对比损失
...
for feta in range(len(paths)):
    ...
    for exp_path in exps:
        ...
        # 定义模型
        model = social_stgcnn_with_contrastive(
        ...
        # 修改状态字典，修正键名
        if 'tpcnn_ouput.weight' in state_dict:
            state_dict['tpcnn_output.weight'] = state_dict.pop('tpcnn_ouput.weight')
        if 'tpcnn_ouput.bias' in state_dict:
            state_dict['tpcnn_output.bias'] = state_dict.pop('tpcnn_ouput.bias')
